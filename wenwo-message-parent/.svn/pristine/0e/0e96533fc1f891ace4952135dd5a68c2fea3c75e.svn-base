$(function(){
	...
	var REQUEST_URLS = {
			...
			"URL_SUBMIT_PIC_TEMPLATE":"/template.do?act=savePicTemplateToShare",
			"URL_GET_PIC_TEMPLATE_BY_ID":"/template.do?act=getPicTemplate",
			"URL_GET_MESSAGE_TYPE_LIST":"/share.do?act=getShareList",
	...
	projectSelect.on("click", "li", function(){
		...
		$("#addNewMsgType").attr("href","/share.do?act=addShare&project=" + projectType);
		$("#addNewMsgType").find("span").text(project_name);
		getMessageTypeList(projectType, null);
	});
	...
	function submitTextTemplate(){
		var template_id = text_template.find("#template_id").val();
