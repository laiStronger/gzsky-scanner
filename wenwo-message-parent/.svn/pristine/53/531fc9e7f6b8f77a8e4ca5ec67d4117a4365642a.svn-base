...
import com.wenwo.message.endpoint.impl.ExternalMessageEndpoint;
...
public class MessageChainContainer {
	...
	@Resource
	private IWeiwenDoctorService weiwenDoctorService;
	...
	private Map<String, IMessageEndpointChain> buildMessageChain(MessageTempConfig tempConfig) {
		...
		for (MessageType messageType : messageTypes) {
			...
			if(templateInfo != null && templateInfo.getIsNeed()){
				List<IEndpointFilter> pointFilterList = filterSelector.select(tempConfig.getMessageProjectType().name(), 
                        messageType.getTypeName(), SMSMessageEndpoint.class.getSimpleName());
				SMSMessageEndpoint endpoint=new SMSMessageEndpoint(templateInfo.getTextTemplateId(),
						contentGenerator,messageDao,smsService,aliMessageService,weiwenDoctorService,shardedJedisPool,pointFilterList);
				endpointChain.addEndpoint(endpoint);
			...
			if(templateInfo != null&&templateInfo.getIsNeed()){
				List<IEndpointFilter> pointFilterList = filterSelector.select(tempConfig.getMessageProjectType().name(), 
                        messageType.getTypeName(), ExternalMessageEndpoint.class.getSimpleName());
				ExternalMessageEndpoint endpoint=new ExternalMessageEndpoint(templateInfo.getTextTemplateId(),
						contentGenerator,aliMessageService,pointFilterList);
				endpointChain.addEndpoint(endpoint);
			}
