$(function(){
	...
	function submitMessageType(){
		...
		//阿里信息
		var external_message_send_type=$("#external_message_radio").find("input:radio:checked").val();
		var externalTemplateInfo={};
		externalTemplateInfo.isNeed=false;
		if(external_message_send_type == "normal"){
			externalTemplateInfo.isNeed=true;
			var external_template_text = messageTypeForm.find("#external_template_text").val();
			externalTemplateInfo.external_template_text=external_template_text;
			if(!external_template_text){
				alert("请填写阿里信息内容");
				return;
			}
		}
		messageType.externalTemplateInfo=externalTemplateInfo;
