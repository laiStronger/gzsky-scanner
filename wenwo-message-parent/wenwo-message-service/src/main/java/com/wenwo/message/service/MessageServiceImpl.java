...
//		String msgId = redisUtil.generateID(MSG_ID_KEY);
	...
	@Override
	public String sendExternalMessage(String messageType,
			SubprojectType subprojectType, MessageParameter parameter) {
		...
		try{
		  msgId = GenShortIdUtils.generateShortId();
		  MessageData data = new MessageData(msgId, messageType,null, parameter, subprojectType);
		  redisUtil.pushExternalMessageData(data);
		}catch(Exception e){
			logger.error("发布消息到redis队列失败[{}]", e);
		}
		return msgId;
