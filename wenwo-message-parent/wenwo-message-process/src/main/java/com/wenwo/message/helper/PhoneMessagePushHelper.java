...
public class PhoneMessagePushHelper implements IPhonePushHelper {
	...
	//患者
	@Autowired
	private AppleNotificationServerFactory patientAppleNotificationServerFactory;
	...
	public void pushIphonePatientMessage(MessageRequest request, String msgContent) throws Exception {
		...
		if(drUserDeviceToken == null || StringUtils.isEmpty(drUserDeviceToken.getDeviceToken())){
			logger.warn("患者用户{}缺失token,取消推送", targetUser.getId());
			return;
		}
		...
		logger.info("患者user:{},deviceType:{}, deviceToken:{}", targetUser.getId(), deviceType, deviceToken);
		//每个线程必须创建一个
		PushNotificationManager pushManager = new PushNotificationManager();
		
		AppleNotificationServer ss = patientAppleNotificationServerFactory.getCert(true);
		pushManager.initializeConnection(ss);
		// 发送push消息
		...
		if(notification == null){
			logger.error("推送患者iphone消息失败,messageContent:{},token:{},isProduct:{},用户Id{}", msgContent, deviceToken, true,targetUser.getId());
		} else if(!notification.isSuccessful()){
			logger.error("推送患者iphone消息失败,messageContent:{},token:{},isProduct:{}, notifyResult:{},用户Id{}", msgContent, deviceToken, true, notification, targetUser.getId());
		} else{
			logger.info("推送患者iphone消息成功,messageContent:{},token:{},isProduct:{},用户Id{}", msgContent, deviceToken, true, targetUser.getId());
		}
		...
		AppleNotificationServer testServer = patientAppleNotificationServerFactory.getCert(false);
		...
		if(testNotification == null){
			logger.error("testNotification,推送患者iphone消息失败,messageContent:{},token:{},isProduct:{},用户Id{}", msgContent, deviceToken, true, targetUser.getId());
		} else if(!testNotification.isSuccessful()){
			logger.error("testNotification,推送患者iphone消息失败,messageContent:{},token:{},isProduct:{},notifyResult:{},用户Id{}", msgContent, deviceToken, true, testNotification, targetUser.getId());
		} else{
			logger.info("testNotification推送患者iphone消息成功,messageContent:{},token:{},isProduct:{},用户Id{}", msgContent, deviceToken, true, targetUser.getId());
		}
	}
...
//			jedis.zadd(("aserver_" + subprojectType.name()+ "_" + targetUserId),score, msgContent);
	...
	public void pushPatientAndroidMessageByXinge(MessageRequest request, String msgContent) throws Exception{
		...
		//开始调用信鸽内容
		Message message = XingeAppUtil.xingePatientMessage(request,msgContent);
		
		//推送信鸽消息
		JSONObject ret = XingeAppUtil.sendXingeMessage(message,deviceToken,"patient");
