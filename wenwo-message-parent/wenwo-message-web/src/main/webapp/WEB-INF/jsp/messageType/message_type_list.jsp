	...
	<table id="message_type_table" class="table table-bordered table-hover table-condensed" >
		<thead>
			<tr>
				<th width="50">消息类型</th>
				<th width="50">描述</th>
				<th width="50">消息主分类</th>
				<th width="100" value="INSITE_WEB">站内消息模版(Web)</th>
				<th width="100" value="INSITE_APP">站内消息模版(手机)</th>
				<th width="100" value="PUSH">手机PUSH模版</th>
				<th value="WEIBO">微博通知模板</th>
				<th value="PRIMESSAGE">私信通知模板</th>
				<th value="SMS">短信通知模板</th>
				<th value="EXTERNAL">信息模板</th>
				<th>操作</th>
			</tr>
		</thead>
		<tbody>
			...
			<tr id="${messageType.id}" <c:if test="${messageType.status eq 'NEED_LOAD' }">class="warning"</c:if> >
				...
				<td info_type="PRIMESSAGE" message_type_id="${messageType.id}">
			      <label class="radio">
			        <input type="radio" value="" name="mpri_${messageType.id }" id="mpri_${messageType.id }" <c:if test="${not empty messageType.priMessageTemplateInfo && messageType.priMessageTemplateInfo.isNeed}">checked</c:if>>
			      	...
			      	<c:choose>
			      		<c:when test="${not empty messageType.priMessageTemplateInfo && not empty messageType.priMessageTemplateInfo.textTemplateId && messageType.priMessageTemplateInfo.isNeed}">  <!-- && messageType.priMessageTemplateInfo.isNeed -->
				...
				<!-- 私信消息结束 -->
				<!-- 短信消息 -->
				<td info_type="SMS" message_type_id="${messageType.id}">
			      <label class="radio">
			        <input type="radio" value="" name="msms_${messageType.id }" id="msms_${messageType.id }" <c:if test="${not empty messageType.smsTemplateInfo && messageType.smsTemplateInfo.isNeed}">checked</c:if>>
			                   短信消息
			      </label>
			        <c:choose>
			      		<c:when test="${not empty messageType.smsTemplateInfo && not empty messageType.smsTemplateInfo.textTemplateId && messageType.smsTemplateInfo.isNeed}">  <!-- && messageType.priMessageTemplateInfo.isNeed -->
						<button templateId="${messageType.smsTemplateInfo.textTemplateId }" class="btn btn-success btn-mini text_template_change">文字</button>
			      		</c:when>
			      		<c:otherwise>
			      			<button templateId="" class="btn btn-mini text_template_change">文字</button>
			      		</c:otherwise>
			      ...
			      <label class="radio">
			        <input type="radio" value="" name="msms_${messageType.id }" id="msmsallnot_${messageType.id }" <c:if test="${(empty messageType.smsTemplateInfo || !messageType.smsTemplateInfo.isNeed)}">checked</c:if>>
			                      不需要
			      </label>
			      	
				</td>
				<!-- 短信消息结束 -->
			   <td info_type="EXTERNAL" message_type_id="${messageType.id}">
			      <label class="radio">
			        <input type="radio" value="" name="mexter_${messageType.id }" id="mexter_${messageType.id }" <c:if test="${not empty messageType.externalTemplateInfo && messageType.externalTemplateInfo.isNeed}">checked</c:if>>
			                  外部信息模板
			      </label>
			        <c:choose>
			      		<c:when test="${not empty messageType.externalTemplateInfo && not empty messageType.externalTemplateInfo.textTemplateId && messageType.externalTemplateInfo.isNeed}">  <!-- && messageType.priMessageTemplateInfo.isNeed -->
						<button templateId="${messageType.externalTemplateInfo.textTemplateId }" class="btn btn-success btn-mini text_template_change">文字</button>
			      		</c:when>
			      		<c:otherwise>
			      			<button templateId="" class="btn btn-mini text_template_change">文字</button>
			      		</c:otherwise>
			      ...
			      <label class="radio">
			        <input type="radio" value="" name="mexter_${messageType.id }" id="mexterallnot_${messageType.id }" <c:if test="${(empty messageType.externalTemplateInfo || !messageType.externalTemplateInfo.isNeed)}">checked</c:if>>
			                      不需要
			      </label>
